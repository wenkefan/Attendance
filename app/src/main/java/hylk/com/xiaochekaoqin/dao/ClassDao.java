...
//		db.close();
    ...
    public Child queryChild(int userId) {
        SQLiteDatabase db = openHelper.getReadableDatabase();
        Child bean = new Child();
        Cursor cursor1 = db.rawQuery(
                "select * from AttendanceUser where UserId = ? and State = ? and UserType = ? ",  // and UserType = ? usertype 幼儿1  老师2
                new String[]{String.valueOf(userId), String.valueOf(1), String.valueOf(1)});
        if (cursor1.moveToNext()) {
            // 幼儿名称
            bean.userid = userId;
            bean.name = cursor1.getString(cursor1.getColumnIndex("UserName"));
            ...
            Cursor cursor3 = db.rawQuery(
                    "select * from Card where State = ? and UserId = ?", //CardType 幼儿1 老师2 家长3
                    new String[]{String.valueOf(1), String.valueOf(userId)});
            if (cursor3.moveToNext()){
                bean.sacrd = cursor3.getString(cursor3.getColumnIndex("CardNo"));
            }
            if (cursor3 != null){
                cursor3.close();
            }
